...
const ReferenceObs = ({ stories, font }) => {
  ...
  const addtoItemEls = (el, id) => {
    if (
      el &&
      el !== null &&
      !itemEls.current.some((obj) => obj.id === id)
    ) {
  ...
  useEffect(() => {
    if (stories && selectedStory !== undefined) {
      const currentRef = itemEls.current.filter(
        (obj) => obj.id === selectedStory,
      )[0]?.el;
      if (currentRef) {
      ...
      {isLoading === false ? (
        <>
          {stories.map((story, index) => (
            <div
              key={story.id}
              className={`grid grid-cols-12 auto-cols-max gap-5 p-0  ${story.id === selectedStory && 'bg-light'
                } ${story.id % 2 ? 'bg-gray-50' : 'bg-white'}`}
              ...
              {Object.prototype.hasOwnProperty.call(
                ...
                'title',
                  ...
                  <p
                    className='text-xl text-gray-600 col-span-12 text-center'
                    style={style.bold}>
                    {story.title}
                  </p>
                )}
              ...
              {Object.prototype.hasOwnProperty.call(
                ...
                'end',
                  ...
                  <p
                    className='col-span-8 text-md text-gray-600'
                    style={style.italic}>
                    {story.end}
                  </p>
                )}
