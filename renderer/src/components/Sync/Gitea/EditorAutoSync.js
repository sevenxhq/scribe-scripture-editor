...
function AutoSync({ selectedProject }) {
  ...
  const getGiteaUsersList = async () => new Promise(async (resolve, reject) => {
    ...
    const filePath = `autographa/users/samuel/ag-user-settings.json`;
    const { data: agSettings, error } = await supabaseStorage().download(filePath);
    const json = JSON.parse(await agSettings.text());
    if (json) {
      console.log('user json : ', json.sync?.services?.door43)
      resolve(json.sync?.services?.door43);
    }
            ...
            {uploadDone ? (
              ...
              : (
                <div
                  ...
                  className={`group ${menuStyles.btn}
              transition-all duration-[${uploadDone ? '0ms' : '2000ms'}]${uploadDone ? 'opacity-0' : 'opacity-100'}`}
                >
