...
export const saveSupabaseProjectsMeta = async (projectMetaObj) => {
  ...
  const translationBurritoChecksAndCreation = async () => {
    ...
    projectMetaObj.importedFiles.forEach((file) => {
      if (!bookAvailable(projectMetaObj.canonSpecification.currentScope, file.id)) {
        checkCanon = true;
        console.log('saveProjectsMeta.js', `${file.id} is not added in Canon Specification or scope`);
        return { type: 'warning', value: `${file.id} is not added in Canon Specification` };
      }
    });

    if (projectMetaObj.call === 'edit' && !checker((projectMetaObj.canonSpecification.currentScope), Object.keys(projectMetaObj.project.type.flavorType.currentScope))) {
      checkCanon = true;
      console.log('saveProjectsMeta.js', 'Not allowed to remove previous scope');
      return { type: 'warning', value: 'You are not allowed to remove previous scope.' };
    }
    ...
    if (checkCanon === false) {
      ...
      if (projectMetaObj.call === 'new') {
        ...
        console.log('saveProjectsMeta.js', 'Fetching the key from the existing Project');
        // from existing metadata
        scope = (projectMetaObj.canonSpecification.currentScope)
          .filter((x) => !(Object.keys(projectMetaObj.project.type.flavorType.currentScope)).includes(x));
        id = Object.keys(projectMetaObj.project?.identification?.primary?.ag);
      ...
      // Create New burrito
      // ingredient has the list of created files in the form of SB Ingredients
      console.log('saveProjectsMeta.js', 'Calling creatVersification for generating USFM files.');
      const ingredient = await createVersificationUSFM(
        currentUser,
        projectMetaObj.newProjectFields,
        projectMetaObj.versificationScheme,
        scope,
      ...
      console.log('saveProjectsMeta.js', 'Calling createTranslationSB for creating burrito.', { ingredient });
      const burritoFile = await createTranslationSB(
        currentUser,
        projectMetaObj.newProjectFields,
        scope,
      ...
      if (error) {
        console.log('saveProjectsMeta.js', error);
