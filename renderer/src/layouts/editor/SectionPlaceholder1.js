        ...
        Object.entries(value).forEach(
          ([, _value]) => {
            Object.entries(_value).forEach(
              ([, resources]) => {
                console.log('resources here', resources, refsHistory);
      ...
      localforage.getItem('projectmeta').then((value) => {
        Object?.entries(value).forEach(
          ([, _value]) => {
            Object?.entries(_value).forEach(
              ([, resources]) => {
                const _refs = resources.project[resources.type.flavorType.flavor.name].refResources;
                if (resources.identification.name.en === _projectname[0]) {
                  console.log('asasasasas', _refs);
                  refsHistory.push(_refs);
