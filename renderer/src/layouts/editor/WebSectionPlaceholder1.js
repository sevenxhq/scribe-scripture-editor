    ...
    const readObs = async () => {
      localforage.getItem('userProfile').then(async (user) => {
        if (_obsNavigation1 && referenceColumnOneData1.refName && referenceColumnOneData1.selectedResource === 'obs') {
          setStories1(await webCore(_obsNavigation1, referenceColumnOneData1.refName, user.user.email));
        }
        if (_obsNavigation2 && referenceColumnOneData2.refName && referenceColumnOneData2.selectedResource === 'obs') {
          setStories2(await webCore(_obsNavigation2, referenceColumnOneData1.refName, user.user.email));
        }
      });
    };
    readObs();
