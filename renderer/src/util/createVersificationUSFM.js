      ...
      if (versification.toLowerCase() === scheme.name) {
        ...
        await books.forEach(async (book) => {
          if (bookAvailable(importedFiles, book)) {
            ...
            await uploadFileToSupabase(`${supabasePath}/${book}.usfm`, file[0].content);
            ...
            if (list[book]) {
              ...
              if (isJsonValid) {
                ...
                await uploadFileToSupabase(`${supabasePath}/${book}.usfm`, reCreatedUsfm);
        ...
        await uploadFileToSupabase(`${supabasePath}/versification.json`, JSON.stringify(file));
        ...
        if (call === 'edit' && currentBurrito?.copyright?.shortStatements && (copyright.licence).length <= 500) {
          ...
          console.log('createVersificationUSFM.js', 'Won\'t create license.md file in ingredients and update the current shortStatements');
        } else {
          logger.debug('createVersificationUSFM.js', 'Creating license.md file in ingredients');
          console.log('createVersificationUSFM.js', 'Creating license.md file in ingredients');

          await uploadFileToSupabase(`${supabasePath}/license.md`, copyright.licence);
          ...
          console.log('createVersificationUSFM.js', 'Creating ag-settings.json file in ingredients');

          await uploadFileToSupabase(`${supabasePath}/ag-settings.json`, JSON.stringify(settings));
